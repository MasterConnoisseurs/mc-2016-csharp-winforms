...
{
    public partial class InventoryAmountForm : Form
    {
                ...
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    const string query = @"
                        ...
                        FROM
                            inventory AS i
                        INNER JOIN 
                            inventory_prices AS ip ON i.Id = ip.inventory_id
                        WHERE
                            i.is_deleted = FALSE
                            AND i.is_active = TRUE
                            AND ip.Id = (SELECT ip2.Id 
                                         FROM inventory_prices AS ip2
                                         WHERE ip2.inventory_id = i.Id
                    ...
                    using (var command = new MySqlCommand(query, connection))
                    {
                        using (var reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                {
                                    ItemId = reader.GetInt32("inventory_id"),
                                    ItemName = reader.GetString("inventory_name"),
        ...
        private void submit_button_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show(@"Invalid inventory item selected.",
