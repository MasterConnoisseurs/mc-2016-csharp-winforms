...
namespace MC.LaundryShop.App.Forms
{
    public partial class CustomerForm : Form
    {
        ...
        public CustomerForm(long? formId, long? userId)
        {
            InitializeComponent();
            _formId = formId;
            _userId = userId;   
            ...
            {
                label3.Text = @"Update your customer record here.";
                submit_button.Text = @"Update";
                LoadData();
            }
            else
            {
                label3.Text = @"Save your customer record here.";
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    connection.Open();
                    const string query = "SELECT first_name, middle_name, last_name, suffix, email, contact_number, is_active FROM customers WHERE id = @id;";
        ...
        private bool SaveData()
        {
            ...
            try
            {
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    // --- Duplicate Check ---
                    const string checkDuplicateQuery = "SELECT COUNT(*) FROM customers WHERE full_name = @full_name AND is_deleted = FALSE;";
                    ...
                    const string insertQuery = "INSERT INTO customers (first_name, middle_name, last_name, suffix, full_name, email, contact_number, is_active, created_by, created_datetime, modified_by, modified_datetime) " +
                                              "VALUES (@first_name, @middle_name, @last_name, @suffix, @full_name, @email, @contact_number, @is_active, @user_id, @current_date, @user_id, @current_date);";
                    using (var command = new MySqlCommand(insertQuery, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@full_name", fullName); // Use the already constructed fullName
        ...
        private bool UpdateData()
        {
            ...
            try
            {
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    // --- Duplicate Check ---
                    const string checkDuplicateQuery = "SELECT COUNT(*) FROM customers WHERE full_name = @full_name AND is_deleted = FALSE AND Id != @id ;";
                    using (var checkCommand = new MySqlCommand(checkDuplicateQuery, connection))
                    {
                        checkCommand.Parameters.AddWithValue("@full_name", fullName);
                    ...
                    const string query = "UPDATE customers SET " +
        ...
        {
            if (string.IsNullOrWhiteSpace(customTextbox1.Text) || string.IsNullOrWhiteSpace(customTextbox2.Text))
            {
